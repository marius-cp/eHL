...
ggplot(
  dt %>% 
    filter(test=="eHL"), 
  aes(x=J,y=rej, colour = factor(S), linetype=test)
  )+
  geom_line(size=.55) +
  geom_point(
    dt%>% filter(test=="eHL"), 
  ...
  scale_shape_manual(
    values = c(3,8,17,19,4)
    )

ggsave("plots/Fig_2.pdf", width = 9, height = 12 )
...
ggsave("plots/Fig_3.pdf", width = 9, height = 8 )
